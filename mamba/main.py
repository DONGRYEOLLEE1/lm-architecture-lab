"""Smoke-test & toy training loop for the PyTorch Mamba implementation.

Trains a tiny Mamba on a simple copy task: given a random prefix of tokens,
the model learns to repeat them. This exercises forward, backward, and the
selective scan end-to-end without requiring any dataset.
"""
...
def make_shift_batch(
    ...
    """Trivial pointwise task: y[t] = (x[t] + 1) mod vocab. No long-range memory."""
...
def smoke_test() -> None:
    torch.manual_seed(0)
    args = ModelArgs(d_model=64, n_layer=2, vocab_size=32, d_state=8, d_conv=4)
    model = Mamba(args)
    x = torch.randint(0, args.vocab_size, (2, 16))
    logits = model(x)
    assert logits.shape == (2, 16, args.vocab_size), logits.shape
    logits.sum().backward()
...
def train(
    ...
    args = ModelArgs(d_model=128, n_layer=2, vocab_size=17, d_state=16, d_conv=4)
    ...
    for step in range(1, steps + 1):
        x, y = sampler(batch, seq_len, args.vocab_size, device)
        logits = model(x)
    ...
    model.eval()
    with torch.no_grad():
        x, y = sampler(1, seq_len, args.vocab_size, device)
